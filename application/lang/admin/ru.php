...
define('I_ONDELETE_RESTRICT', 'Удаление запрещено в связи с правилом ON DELETE RESTRICT, установленным для поля "%s" сущности "%s" (`%s`.`%s`), так как минимум один экземпляр этой сущности имеет в этом поле значение, напрямую или каскадно являющееся ссылкой на удаляемую вами запись');