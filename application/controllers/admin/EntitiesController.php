...
class Admin_EntitiesController extends Indi_Controller_Admin_Exportable {
    ...
    public function dropCustom() {
        ...
        m()->nativeCascade(true);

        field('field', 'relation', ['onDelete' => 'CASCADE']);
        field('role', 'entityId', ['onDelete' => 'CASCADE']);
        foreach (m()->all('`fraction` = "custom"') as $entityR) {
    ...
    public function gptAction() {
        ...
        $_SERVER['prompt'] ??= $this->prompt('Describe the app you want to build', [[
            'xtype' => 'textarea',
            'name' => 'description',
            'width' => 600,
        ...
        $resp = (new \Gemini1())->prompt($_SERVER['prompt']);
        file_put_contents('data/gemini/data-structures.txt', $resp);

        // Get php-code generated by GPT from local file
        $code = file_get_contents('data/gemini/data-structures.txt');
        $code = str_replace("\r\n", "\n", $code);

        // Amend php-code to be able to count calls and track progress
        $step = "Creating the app"; $idx = 0;
        $code = preg_replace('~](\)->save\(|)\);$~m', '$0 progress(++$idx, "$step: {percent}%");', $code, count: $total);

        // Write amended php-code to a file and execute it
        file_put_contents('data/gemini/data-structures.php', $code);
        progress("$step...", $total);
        include ('data/gemini/data-structures.php');
